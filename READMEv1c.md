...
## 6.3 `Iriven\WorldDatasets\CountryInfo`
...
```php
$names = $worldDatasets->countries()->map(fn (Iriven\WorldDatasets\CountryInfo $country) => $country->name());

$eurCountries = $worldDatasets->countries()->filter(
    fn (Iriven\WorldDatasets\CountryInfo $country) => $country->hasCurrency('EUR')
);

$total = $worldDatasets->countries()->reduce(
    fn (int $carry, Iriven\WorldDatasets\CountryInfo $country) => $carry + 1,
    0
);
```
